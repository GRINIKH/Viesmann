...
class Paths:
    ...
    def __post_init__(self):
        ...
        self.input_file = self.INPUT_DIR / "VS_Negativ_Liste.csv"
...
class ScraperSettings: 
    ...
    MAX_RETRIES: int = 3; RETRY_DELAY_SECONDS: int = 5; HEADLESS_BROWSER: bool = False
...
class ViessmannScraper:
    ...
    def _load_articles_from_csv(self) -> list[str]:
        try:
            ...
            for encoding in encodings:
                try:
                    # Lese CSV mit Header und verwende "Artikelnummer" Spalte
                    df = pd.read_csv(self.config.paths.input_file, sep=';', dtype=str, encoding=encoding)
            ...
            # Prüfe ob "Artikelnummer" Spalte existiert
            if 'Artikelnummer' not in df.columns:
                logging.error("❌ Spalte 'Artikelnummer' nicht in CSV gefunden")
                return []
                
            articles_raw = df['Artikelnummer'].dropna().unique().tolist()
            logging.info(f"📄 {len(articles_raw)} einzigartige Artikel aus CSV geladen.")
            return articles_raw
            
        except FileNotFoundError: 
            logging.error(f"❌ Input-Datei nicht gefunden: {self.config.paths.input_file}")
            return []
        except pd.errors.EmptyDataError: 
            logging.error(f"❌ Input-Datei ist leer: {self.config.paths.input_file}")
            return []
        except Exception as e: 
